...
class MoReportPdfExport:
    ...
    @staticmethod
    def sweep_expired_files(*, db: Session) -> int:
        """
        ลบไฟล์ PDF ที่เลย expires_at แล้วออกจากดิสก์

        เรียกจาก worker เป็นระยะ (sweep interval) เพื่อเก็บกวาดไฟล์เก่า
        โดยไม่เปลี่ยนค่าต่าง ๆ ในตาราง (ลบไฟล์โดยไม่แตะ record)
        """
        expired_jobs = db.execute(
            select(MoReportExportJob).where(
                MoReportExportJob.expires_at.is_not(None),
                MoReportExportJob.expires_at <= datetime.now(),
                MoReportExportJob.file_relative_path.is_not(None),
            )
        ).scalars().all()

        removed_count = 0

        for export_job in expired_jobs:
            export_root = MoReportPdfExport._get_export_root()
            file_path = (
                export_root / str(export_job.file_relative_path).lstrip("/\\")
            ).resolve()

            try:
                file_path.relative_to(export_root)
            except ValueError:
                # path หลุดออกจาก export_root → ไม่แตะไฟล์
                continue
            ...
            try:
                ...
                logger.exception(
                    "ไม่สามารถลบไฟล์ PDF ที่หมดอายุได้: %s",
                    file_path,
                )
                continue
        ...
        if removed_count:
            logger.info("sweep_expired_files: ลบไฟล์ที่หมดอายุ %s ไฟล์", removed_count)
